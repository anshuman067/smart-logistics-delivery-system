...
razorpay_client = razorpay.Client(auth=("rzp_test_WXRNMLHZxsiQwu", "YZQYJnA9KdWObC0zz4ImSYYy"))
...
def predict():
    if request.method == "POST":
        try:
            ...
            pass_values = np.array([[source_center_enc[0], source_name_enc[0], destination_center_enc[0],
                                     destination_name_enc[0], float(actual_distance_to_destination_scaled),
                                     source_states_enc[0], destination_states_enc[0], int(od_start_time_hour),
                                     float(rout_type)]])
            ...
            # Calculate fast delivery time (30% faster)
            fast_minutes = predicted_time[0] * 0.7
            fast_hours = int(fast_minutes // 60)
            fast_mins = round(fast_minutes % 60)
            fast_time_display = f"{fast_hours} hours and {fast_mins} minutes"
            ...
            return render_template('result.html', trip_id=trip_id, source_cities=source_cities,
                                   source_states=source_states,
                                   destination_cities=destination_cities, destination_states=destination_states,
                                   actual_time=eta, rout_type=rout_type, trip_creation_time=trip_creation_time,
                                   actual_time_display=actual_time_display,
                                   actual_distance=actual_distance_to_destination,
                                   fast_time_display=fast_time_display)
...
def payment():
    try:
        ...
        print(
            f"Received: Trip ID={trip_id}, Distance={actual_distance}, Route={rout_type}, Weight={weight}, Type={delivery_type}")
        ...
        time_hours = 5 if rout_type == 0 else 8
        price = fixed_charge
        ...
        rge + (actual_distance * base_rate) + (weight * weight_rate) + (time_hours * time_rate)
        price = int(price * 100)
        ...
        return "Something went wrong during payment setup."
